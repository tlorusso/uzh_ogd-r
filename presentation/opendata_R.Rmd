...
# plot <- plotdata %>% 
#   e_charts(ja_anteil) %>% 
#   e_scatter(sp_anteil,einwohner, scale="/10000+10") %>% 
#   e_visual_map(einwohner) %>% # scale color
#   e_legend(FALSE) %>% 
#   e_lm(sp_anteil~ja_anteil)
...
# plot3 <-plotdata %>% 
#   e_charts(ja_anteil) %>% 
#   e_scatter(sp_anteil,einwohner, scale="/10000+10", bind = GEBIET_NAME) %>% 
#   e_visual_map(einwohner) %>% # scale color
#   e_lm(sp_anteil~ja_anteil) %>%
#   e_legend(FALSE) %>% 
#   # e_tooltip(formatter = htmlwidgets::JS("function(params){ return('<strong>' + params.name +'</strong><br /> Ja-Anteil: ' +params.value[0] +'<br />SP: ' + params.value[1])}"))
# # %>%
#   e_toolbox_feature(feature = "saveAsImage",title="Bild speichern") %>%
#   e_toolbox_feature(feature = "dataView",title="Daten", lang="") %>%
#   e_title("Zustimmung zur VI bezahlbare Kinderbetreuung & SP-Wähleranteil","SP Wähleranteil bei den Nationalratswahlen 2015") %>% 
#   e_tooltip(formatter = htmlwidgets::JS("function(params){ return('wt: ' + params.value[0] + '<br />mpg: ' + params.value[1])}"))
